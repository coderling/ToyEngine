...
GraphicsCommandList::GraphicsCommandList() { Create(0, COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_DIRECT); }

GraphicsCommandList::GraphicsCommandList(const uint32_t& node_mask, const COMMAND_LIST_TYPE& t) { Create(node_mask, t); }
...
void GraphicsCommandList::Create(const uint32_t& node_mask, const COMMAND_LIST_TYPE& t)
{
    ...
    ASSERT_SUCCEEDED(device->CreateCommandList(node_mask, t, command_allocator.Get(),
