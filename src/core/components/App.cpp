...
void App::Finalize() { system_mgr->Destroy(); }

void App::Tick()
{
    system_mgr->Tick();
    IApp::env->pipeline->Tick();
}

bool App::IsQuit() const { return quit; }

void App::Quit() { quit = true; }

const AppArgs* App::GetArgs() const noexcept { return args.get(); }
