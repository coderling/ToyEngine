...
public:
	void Load() override
	{
		...
		ASSERT_SUCCEEDED(device->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), MY_IID_PPV_ARGS(&root_signature)));
		...
		const auto& wpath = Toy::s2ws(shader_path).c_str();
		ASSERT_SUCCEEDED(D3DCompileFromFile(wpath, nullptr, nullptr, "VSMain", "vs_5_0", compileflags, 0, &vertex_shader, nullptr));
		ASSERT_SUCCEEDED(D3DCompileFromFile(wpath, nullptr, nullptr, "PSMain", "ps_5_0", compileflags, 0, &pixel_shader, nullptr));
