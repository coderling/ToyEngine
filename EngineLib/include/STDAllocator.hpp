...
{
template <typename T>
typename std::enable_if<std::is_destructible<T>::value, void>::type Destruct(
    T* ptr)
{
    ptr->~T();
}
...
struct STDAllocator
{
   public:
    ...
    STDAllocator(AllocatorType& allocator, const char* description,
                 const char* filename, const int& line) noexcept
        : allocator(allocator)
    ...
    T* allocate(std::size_t count)
    {
        ...
        return reinterpret_cast<T*>(allocator.Allocate(
            count * sizeof(T), description, filename, linenumber));
    }
...
#define STD_ALLOCATOR(Type, AllocatorType, Allocator, Description)             \
    STDAllocator<Type, AllocatorType>(Allocator, Description, __FILE__,        \
                                      __LINE__)
...
#define STD_ALLOCATOR_RAWMEM(Type, Allocator, Description)                     \
    STDAllocatorRawMem<Type>(Allocator, Description, __FILE__, __LINE__)
...
struct STDDeleter
{
    ...
    STDDeleter(STDDeleter&& rhs) noexcept : allocator(rhs.allocator)
    {
